...
let delFood = async (_id) => {
  // _id 就是要删除的菜品主键id
  let result = await CityModel.deleteOne({ name: _id })
  return result
}
// 修改城市数据
let updateFood = async (_id, updateInfo) => {
  // _id 要修改的主键id  updateInfo 修改的目标数据
  let result = await CityModel.update({ "name": _id }, updateInfo)
  return result
}
// 分页查询
...
let findFoodByKw = async (kw, page, pageSize) => {
  ...
  // 满足条件的所有数据
  let allFood = await CityModel.find({ $or: [{ desc: { $regex: regex } }, { name: { $regex: regex } }] })
  let allCount = allFood.length
  // 分页后满足关键字的数据
